#Exécuter ce script dans le répertoire ou il y a le mineur principal de la pool APOOL.IO

########################################################################################
#Veuillez modifier en fonction de votre choix/configuration
#Cette configuration est prepare pour miner du XELIS pendant la periode IDLE de QUBIC.

cd /tmp && rm -rf /tmp/rigel-1.19.1-linux /tmp/rigel-1.19.1-linux.tar.gz && cd /hive/miners/custom/apoolminer_hiveos

#Récupération du nom du RIG
hostname=$(hostname)

#Nom du mineur IDLE
idle_miner=rigel

#Lien de download du miner IDLE
idle_miner_link=https://github.com/rigelminer/rigel/releases/download/1.19.1/rigel-1.19.1-linux.tar.gz

#Download du miner IDLE
cd /tmp && wget $idle_miner_link && tar -xvf rigel-1.19.1-linux.tar.gz && cd rigel-1.19.1-linux && cp -f rigel /hive/miners/custom/apoolminer_hiveos && cd /tmp && rm -rf /tmp/rigel-1.19.1-linux /tmp/rigel-1.19.1-linux.tar.gz && cd /hive/miners/custom/apoolminer_hiveos

#Commande du miner IDLE
third_miner="$idle_miner"
third_cmd="./$idle_miner -a xelishashv2 -o stratum+tcp://fr.vipor.net:5077 -u xel:kprzhxdp594m5zczayy9d8cxg3kqu978mwqf9hh8qgume3njc97szqqyve0zt5c0pf0st -w $hostname"

# Scripts d'overclocking à exécuter
ocdebut="/home/user/ocdebut.sh"	# Script à exécuter quand le mining_seed est idle
    ...
    if [ -e "$third_miner" ]; then
        ...
        if [ "$url_code" -eq 200 ]; then
            ...
            if [ -z "$mining_seed" ]; then
                ...
                if ! pgrep -f "$idle_miner" > /dev/null; then
                ...
                # Si le mining_seed n'est plus celui attendu, arrêter third_cmd (tous les processus $idle_miner)
